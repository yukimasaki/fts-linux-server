...
容量不足のエラーが発生すると以下のようなメール通知が送信されます。  
このエラーが発生した場合は、不要なバックアップを削除するとともに、バックアップ保持数を減らしてただちにバックアップを実行してください。
```
ステータス: 失敗
結果: バックアップ先のディスク容量が不足しています
ディスク容量不足のため、不要なバックアップを削除してバックアップ保持数を調整してください。
```
...
必要な項目を入力し`インポート`をクリックします。
![](images/restore_machines_002.png)